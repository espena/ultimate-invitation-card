...
void ssd1306_display_text(SSD1306_t * dev, int page, const char * text, int text_len, bool invert) {
  ssd1306_display_text_offset(dev, page, text, text_len, invert, 0, NULL);
}

void ssd1306_display_text_offset(SSD1306_t * dev, int page, const char * text, int text_len, bool invert, int offset, const char * text_below)
{
  ...
  for (uint8_t i = 0; i < _text_len; i++) {
    ...
    if( offset > 0 && text_below != NULL ) {
	    uint8_t image_below[8];
      memcpy(image_below, font8x8_basic_tr[(uint8_t)text_below[i]], 8);
      ssd1306_offset( image, image_below, 8, offset );
    }
    if( invert ) {
...
void ssd1306_offset(uint8_t *buf, uint8_t *buf_below, size_t blen, int offset)
{
	uint8_t wk, wb;
	for(int i=0; i<blen; i++){
		wk = buf[i];
    wb = buf_below[i];
    buf[i] = ( wk >> offset ) | ( ( wb << ( 8 - offset ) ) & ( 0xff << ( 8 - offset ) ) );
	}}

...
const char *captive_portal_dns::m_component_name = "captive_portal_dns";

captive_portal_dns::captive_portal_dns() : m_message_queue( nullptr ),
                                           m_sock( -1 )
{
  ...
  m_task_params.stack_buffer = ( StackType_t * ) heap_caps_malloc( m_component_stack_size,
                                                                   MALLOC_CAP_SPIRAM );
  xTaskCreateStatic( &captive_portal_dns::task_loop,
                     m_component_name,
...
void captive_portal_dns::task_loop( void *arg ) {
...
void captive_portal_dns::server_loop() {
...
void captive_portal_dns::on_message( captive_portal_dns_task_message msg, void *arg ) {
...
bool captive_portal_dns::packet_valid( uint8_t * udp_msg, uint32_t len ) {
...
std::string captive_portal_dns::get_label_part( uint8_t *&p, const uint8_t *eop ) {
  std::string str = "";
  const uint8_t len = *p++;
...
std::string captive_portal_dns::get_label( uint8_t *udp_msg, uint8_t *&p, uint32_t len ) {
...
void captive_portal_dns::put_label( std::string label,
                                    uint8_t *&buf,
                                    const uint8_t *eob )
{
  uint8_t *lenptr = buf++;
...
void captive_portal_dns::on_receive( struct sockaddr_in *from,
                                     uint8_t *udp_msg,
                                     uint32_t len )
{
...
void captive_portal_dns::create_sock() {
  do {
...
void captive_portal_dns::bind_sock() {
...
void captive_portal_dns::init_sync() {
  ESP_LOGI( m_component_name, "Initialize DNS server" );
  create_sock();
  bind_sock();
}

void captive_portal_dns::start_sync() {
  ESP_LOGI( m_component_name, "Starting DNS server" );
  server_loop();
}

void captive_portal_dns::stop_sync() {
  ESP_LOGI( m_component_name, "Stopping DNS server" );
}

void captive_portal_dns::enqueue_simple_message( captive_portal_dns_task_message msg ) {
